...
s3_bucket = "socialdistancingtracker"
...
ip_address = headcount_video #'rtsp://10.132.173.28:8080/h264_ulaw.sdp'
j = 0
while True:
    i=0
    a=[]
    ...
    while (cap.isOpened()):
        ret,frame = cap.read()
        if ret:
            cv2.imwrite('temp/frame'+str(i)+'.jpeg',frame)
            a.append(i)
            i+= (fps * time_cut)
            cap.set(1, i)
        else:
    ...
    for i in a:
        file_name = 'temp/frame'+str(i)+'.jpeg'
        key_name = 'temp/frame'+str(i)+'.jpeg'
